...
$app->get('/', function (Request $request, Response $response, $args) {
    $response->getBody()->write("sios");
    return $response;
});

//Endpoint login de usuario
$app->get('/getUser/{id}[/{myContrasena}]', function ($request, $response, $args) {  //Defino los servicios
	try{
		/* echo "arg: {$args['id']}";
		echo "arg2: {$args['myContrasena']}"; */
		$db =  getDB(); //Carga los datos
		$sth = $db->prepare("SELECT id, contrasena, rol from usuarios where id = :id");//Consulta
		$sth->bindParam(":id", $args["id"], PDO::PARAM_STR); //recuperamos el identificador (sustituir el id por el parametro que hemos recibido) y PARAM_INT le especificamos que es de tipo entero
		$sth->execute(); //Ejecutamos la consulta
		$test = $sth->fetchAll(PDO::FETCH_ASSOC);//Guardar los resultados de la consulta
		if($test){
			
			/* echo "entro";
			echo "myContraseña: {$args['myContrasena']}";
			$hashed = password_hash($args['myContrasena'], PASSWORD_DEFAULT);
			echo "Hashed: $hashed"; */
			$rolUser = $test[0]['rol'];
			/* echo "Su rol es: $rolUser"; */


			//if(hash_equals($args['id2'], $test[0])){
			//if($hashed === $test[0][0]){
			if ($rolUser === 'master' and password_verify($args['myContrasena'], $test[0]['contrasena'])) { //Verficia el password enviado por el usuario ($pass)
				//Por el que devuelve la consulta $user['clave'] que es un hash
				/* echo "excelente"; */
				$response->getBody()->write('{"ok":"ok"}');
			...
			/* if (password_verify($args["id2"], $test['usrContrasena'])) {
				
			} */
		}
			if ($rolUser === 'aprendiz' and password_verify($args['myContrasena'], $test[0]['contrasena'])) {
				$response->getBody()->write('{"ok":"aprendiz"}');
				/* $response->getBody()->write(json_encode($test)); */ //write Escribe la respuesta como texto, pero necesito un Json
				$db = null;//Cerrar la conexion con la base de datos
			}
